...
write_rds(copd,'copd_modeling_final.rds')
# garbage
######################
# copd %>%
...
library(frailtypack)
select <-dplyr::select
copd <- read_rds('copd_modeling_final.rds')
...
tmp <- copd %>% group_by(ID) %>% tally()
ID.1 <- tmp %>% filter(n ==1) %>% select(ID) %>% unlist() %>% as.vector()

# copd.real <- copd
# copd <- copd %>% 
#   filter(!(ID %in% ID.1))
  # mutate(trt =as.character(trt),
  #        trt=ifelse(trt=='Sal_Flu','placebo','Sal'),
  #        trt=as.factor(trt))
